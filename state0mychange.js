...
demo.state0.prototype = {
    ...
    update: function(){
        ...
        game.physics.arcade.collide(npc,detective,interactionHandler(detective,npc,plink));
...
function interactionHandler(detective,npc,sound){
    if(Math.abs(detective.x-npc.x)<80 && Math.abs(detective.y-npc.y)<80){
            if(game.input.keyboard.isDown(Phaser.Keyboard.E) && counter<1){
                ...
                option1 = game.add.text(0,830,npc1Questions[0],{fontsize: '20px',fill: '#ffffff'})
                option2 = game.add.text(0,860,npc1Questions[1],{fontsize: '20px',fill: '#ffffff'})
                option3 = game.add.text(0, 890, npc1Questions[2], {fontsize: '20px',fill: '#ffffff'})
                option1.inputEnabled=true;
                option2.inputEnabled=true;
                option3.inputEnabled=true;
                option1.events.onInputDown.add(function(){option1.addColor('#000000',0); sound.play();});
